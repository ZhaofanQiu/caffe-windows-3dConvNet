  ...
  for (int n = 0; n < num_; ++n) {
	for (int g=0; g<filter_group_; ++g){
      caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_ / filter_group_,
        ...
        (Dtype)1., weight + g * weight_offset_, bottom_data + bottom[0]->offset(n) + output_offset_ * g,
        (Dtype)0., col_data + g * col_offset_);
    }
...
void Deconvolution3DLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
	  ...
	  vol2col_gpu(top_diff + top[0]->offset(n), channels_, length_, height_,
		  width_, kernel_size_, kernel_depth_, pad_, temporal_pad_, stride_,
		  temporal_stride_, col_diff);

		  // gradient w.r.t. weight. Note that we will accumulate diffs.
		  for (int g = 0; g<filter_group_; ++g){
			  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, num_output_ / filter_group_,
	  ...
	  if (propagate_down) {
		  ...
		  for (int g = 0; g<filter_group_; ++g){
			  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_ / filter_group_,
