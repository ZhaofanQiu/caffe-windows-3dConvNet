...
Dtype SliceLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (slice_dim_ == 0) {
    ...
    for (int i = 0; i < top->size(); ++i) {
      ...
      caffe_gpu_copy(blob->count(), bottom_data + bottom[0]->offset(offset_num),
                 top_data);
      offset_num += blob->num();
    }
    ...
    for (int i = 0; i < top->size(); ++i) {
      ...
      for (int n = 0; n < num_; ++n) {
        ...
        caffe_gpu_copy(num_elem, bottom_data + bottom[0]->offset(n, offset_channel),
...
void SliceLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (slice_dim_ == 0) {
    ...
    for (int i = 0; i < top.size(); ++i) {
      ...
      caffe_gpu_copy(blob->count(), top_diff,
    ...
    for (int i = 0; i < top.size(); ++i) {
      ...
      for (int n = 0; n < num_; ++n) {
        ...
        caffe_gpu_copy(num_elem, top_diff + blob->offset(n),
