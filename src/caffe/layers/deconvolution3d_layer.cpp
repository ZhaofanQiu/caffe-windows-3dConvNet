...
void Deconvolution3DLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  col_buffer_.Reshape(
      1, channels_ * kernel_depth_ * kernel_size_ * kernel_size_, length_out_, height_out_, width_out_);


  bias_term_ = this->layer_param_.convolution_param().bias_term();

  // Figure out the dimensions for individual gemms.
  kernel_dim_ = channels_ * kernel_depth_ * kernel_size_ * kernel_size_;
  ...
  for (int n = 0; n < num_; ++n) {
	  ...
	  for (int g = 0; g < filter_group_; ++g) {
		  caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_ / filter_group_, 
			  conv_out_spatial_dim_, num_output_ / filter_group_,
			  (Dtype)1., weight + weight_offset_ * g, bottom_data + bottom[0]->offset(n) + output_offset_ * g,
			  (Dtype)0., col_data + col_offset_ * g);

	  }
	  //Second, col2vol
	  col2vol_cpu(col_data, channels_, length_, height_, width_, kernel_size_, kernel_depth_, pad_,
...
void Deconvolution3DLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
	...
	// we will need to recompute them.
	  vol2col_cpu(top_diff + top[0]->offset(n), channels_, length_, height_,
		  width_, kernel_size_, kernel_depth_, pad_, temporal_pad_, stride_,
		  temporal_stride_, col_diff);

	// gradient w.r.t. weight. Note that we will accumulate diffs.
	for (int g=0; g<filter_group_; ++g){
		caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, num_output_ / filter_group_, 
			kernel_dim_ / filter_group_, conv_out_spatial_dim_ ,
				(Dtype)1., bottom_data + (*bottom)[0]->offset(n) + g * output_offset_,
				col_diff + col_offset_ * g, (Dtype)1.,
	...
	if (propagate_down) {
	  ...
	  for (int g=0; g<filter_group_; ++g){
	    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_ / filter_group_, 
			conv_out_spatial_dim_, kernel_dim_ / filter_group_,
	      (Dtype)1., weight + g * weight_offset_,
		  col_diff + g * col_offset_,
