...
void SliceLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
	...
	CHECK_GT(top->size(), 1) <<
		"SliceLayer takes at least two blobs as output.";
	CHECK_EQ(bottom.size(), 1) <<
		"SliceLayer takes a single blob as input.";

	slice_dim_ = this->layer_param_.slice_param().slice_dim();
	CHECK_GE(slice_dim_, 0);
	CHECK_LE(slice_dim_, 1) << "Can only slice num and channels";
	slice_point_.clear();
	std::copy(this->layer_param_.slice_param().slice_point().begin(),
		this->layer_param_.slice_param().slice_point().end(),
	...
	if (slice_point_.size() != 0) {
		...
		if (slice_dim_ == 0) {
			...
			for (int i = 0; i < top->size(); ++i) {
				(*top)[i]->Reshape(slices[i], channels_, length_, height_, width_);
				count_ += (*top)[i]->count();
			}
		}
		else {
			...
			for (int i = 0; i < top->size(); ++i) {
				(*top)[i]->Reshape(num_, slices[i], length_, height_, width_);
				count_ += (*top)[i]->count();
			}
		}
	}
		...
		for (int i = 0; i < top->size(); ++i) {
			(*top)[i]->Reshape(num_, channels_, length_, height_, width_);
			count_ += (*top)[i]->count();
		}
	}
...
Dtype SliceLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (slice_dim_ == 0) {
    ...
    for (int i = 0; i < top->size(); ++i) {
      Blob<Dtype>* blob = (*top)[i];
      Dtype* top_data = blob->mutable_cpu_data();
      const int num_elem = blob->channels() * blob->length() * blob->height() * blob->width();
