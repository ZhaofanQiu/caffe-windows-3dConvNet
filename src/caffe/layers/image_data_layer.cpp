...
void ImageDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  use_label_ = this->layer_param_.image_data_param().use_label();
  ...
  if (use_label_){
	  while (infile >> filename >> label) {
		  ...
		  fn_list_.push_back(filename);
		  label_list_.push_back(label);
		  shuffle_index_.push_back(c);
		  c++;
	  }
  }else{
  ...
  LOG(INFO) << "A total of " << shuffle_index_.size() << " images.";
  ...
  Datum datum;
  CHECK(ReadImageToDatum(fn_list_[shuffle_index_[lines_id_]], label_list_[shuffle_index_[lines_id_]],
                         new_height, new_width, &datum));
  // image
  ...
  if (crop_size > 0) {
    (*top)[0]->Reshape(batch_size, datum.channels(), 1, crop_size, crop_size);
    prefetch_data_.reset(new Blob<Dtype>(batch_size, datum.channels(), 1,
                                         crop_size, crop_size));
  } else {
    (*top)[0]->Reshape(batch_size, datum.channels(), 1, datum.height(),
                       datum.width());
    prefetch_data_.reset(new Blob<Dtype>(batch_size, datum.channels(), 1,
  ...
  LOG(INFO) << "output data size: " << (*top)[0]->num() << ","
      ...
      << (*top)[0]->width();
  // label
  (*top)[1]->Reshape(batch_size, 1, 1, 1, 1);
  prefetch_label_.reset(new Blob<Dtype>(batch_size, 1, 1, 1, 1));
  // datum size
  ...
  if (this->layer_param_.image_data_param().has_mean_file()) {
    ...
    //LOG(INFO) << "before " << blob_proto.num() << " " << blob_proto.channels() << " " << blob_proto.length()
    //		<< " " << blob_proto.height() << " " << blob_proto.width();
    if (!blob_proto.has_length())
    	blob_proto.set_length(1);
    //LOG(INFO) << "after " << blob_proto.num() << " " << blob_proto.channels() << " " << blob_proto.length()
    //		<< " " << blob_proto.height() << " " << blob_proto.width();
    data_mean_.FromProto(blob_proto); //error
    ...
    data_mean_.Reshape(1, datum_channels_, 1, datum_height_, datum_width_);
  }
